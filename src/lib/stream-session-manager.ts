...
export function startStream(params: StartStreamParams): void {
  ...
  const stream: ActiveStream = {
    ...
    snapshot: {
      ...
      sessionGrantedTools: existing ? [...existing.sessionGrantedTools] : [],
      activeAgents: existing ? [...existing.activeAgentsMap.values()] : [],
    },
    idleCheckTimer: null,
    ...
    activeAgentsMap: existing?.activeAgentsMap ?? new Map(),
